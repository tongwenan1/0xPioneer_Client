...
import { InnerBuildUpData } from '../../Datas/DataDefine';
import EventMgr from '../../Manger/EventMgr';
import { EventName, ItemType } from '../../Datas/ConstDefine';
...
export  class FactoryInfoUI extends PopUpUI {
    ...
    private _upCfg: InnerBuildUpData = null;

    private _useItemArr: ItemData[] = [];

    private _outputItemArr: ItemData[] = [];
    ...
    refreshUI(data: UserInnerBuildInfo){
        log('initUI ',data);
        this._data = data;
        let up_cfg_arr = GameMain.localDatas.InnerBuildData.innerBuildUp[data.buildID.toString()];
        let up_cfg:InnerBuildUpData = null
        if(up_cfg_arr){
            up_cfg = up_cfg_arr[data.buildLevel];
        }
        this._upCfg = up_cfg;

        if(!data || !up_cfg){
            log('data or up_cfg is null');
            return;
        }

        if(!data || data.buildLevel <= 0){// unlock
        ...
        this.btnUpgrade.interactable = this.canUpgrade;
    ...
    private factoryItemChange(){
        // produce props add and reduce
        // TODO only deal with coin and electricity
        for (const data of this._useItemArr) {
            if(data.itemType == ItemType.Energy){
                EventMgr.emit(EventName.ENERGY_CHANGE,-1*data.cout);
            }
        }


        for (const data of this._outputItemArr) {
            if(data.itemType == ItemType.Coin){
                EventMgr.emit(EventName.COIN_CHANGE,data.cout);
            }
        }
